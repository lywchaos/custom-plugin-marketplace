"""
Inspired by https://github.com/anthropics/claude-cookbooks/blob/main/misc/metaprompt.ipynb
"""
...
def get_client() -> anthropic.Anthropic:
    ...
    if not api_key:
        sys.exit("Error: ANTHROPIC_AUTH_TOKEN environment variable is required")
...
MODEL_NAME = os.environ.get("SKILL_ANTHROPIC_MODEL", "claude-sonnet-4-5")
...
def build_variable_string(variables: list[str]) -> str:
    """Build variable string for the assistant partial response."""
    if not variables:
        return ""
    return "\n".join("{" + var.upper() + "}" for var in variables)


def generate_prompt_template(task: str, variables: list[str]) -> str:
    """Generate a prompt template for the given task and variables."""
    client = get_client()

    prompt = metaprompt.replace("{{TASK}}", task)
    assistant_partial = "<Inputs>"

    variable_string = build_variable_string(variables)
    if variable_string:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate prompt templates using Claude's meta-prompt technique.",
...
Examples:
  ...
  uv run --with anthropic meta-prompt-generate.py -t "Summarize a document" -v DOCUMENT -v MAX_LENGTH
    ...
    parser.add_argument(
        ...
        "--task",
        required=True,
        help="Task description for which to generate a prompt template",
    )
    parser.add_argument(
        "-v",
        "--variable",
        ...
        dest="variables",
        metavar="VAR",
        help="Input variable name (can be specified multiple times)",
    )
...
def main() -> None:
    ...
    response = generate_prompt_template(args.task, args.variables)
    extracted_prompt = extract_prompt(response)

    pretty_print(extracted_prompt)
